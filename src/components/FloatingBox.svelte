...
// @ts-nocheck
    ...
    const submitLocation = async () => {
      try {
        const response = await fetch(`https://api.weatherapi.com/v1/current.json?q=${location}&key=bd8cdf6ec9774d819fe231045240507&aqi=yes`);
        if (!response.ok) {
    ...
    <div class="floating-box">
      ...
      {#if weatherData}
        <div class="weather-info">
          ...
          <p><strong>Air Quality:</strong> {weatherData.current.air_quality["us-epa-index"]} - {gradientMap[airQuality]}</p>
        </div>
      {/if}
      {#if errorMessage}
